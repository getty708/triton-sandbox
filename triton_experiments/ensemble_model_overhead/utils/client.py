...
def create_logdir(args: argparse.Namespace) -> Path:
    logdir = (
        args.logdir / f"b{args.batch_size}-r{args.num_requests}" / f"p{args.pipeline_architecture}"
    )
...
def prepare_image_batch_tensor(
    ...
    image_batch = create_batch_tensor(image_data, batch_size=batch_size, device=torch.device("cpu"))
...
def create_input_tensors_wo_shared_memory(
    ...
    input_tensors = [
        grpcclient.InferInput(DUMMY_IMAGE_INPUT_TENSOR_NAME, batched_image.shape, "FP32"),
    ]
...
def create_input_output_tensors_with_cudashm(
    ...
    input_tensor_name: str = DUMMY_IMAGE_INPUT_TENSOR_NAME,
    output_tensor_name: str = DUMMY_IMAGE_OUTPUT_TENSOR_NAME,
    ...
    shm_ip_handle = cudashm.create_shared_memory_region(input_shm_name, input_byte_size, gpu_id)
    ...
    shm_op_handle = cudashm.create_shared_memory_region(output_shm_name, output_byte_size, gpu_id)
    triton_client.register_cuda_shared_memory(
        output_shm_name,
    ...
    input_tensors = []
    input_tensors.append(grpcclient.InferInput(input_tensor_name, batched_image.shape, input_dtype))
